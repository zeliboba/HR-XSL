...
<!-- 
    ...
    XSLT versions tested and do not seem to work
           * Unicorn 1.03 problems with call-template and xsl:choose
           * XXXXXX for Java
           * Infoteria iXSLT 2.0c
	...
	<xsl:template match="sch:schema | schema">
		<axsl:stylesheet version="1.0">
			...
			<xsl:for-each select="sch:ns | ns">
	 		...
	 		-->
			<!-- -->
			<xsl:for-each select="sch:ns[@prefix = ''] | ns[@prefix = '']">
				<xsl:if test="position() = 1">
					<xsl:attribute name="dummy-for-xmlns" namespace="{@uri}"/>
				</xsl:if>
			</xsl:for-each>
			<xsl:for-each select="sch:ns[@prefix != ''] | ns[@prefix != '']">
				<xsl:attribute name="{concat(@prefix,':dummy-for-xmlns')}" namespace="{@uri}"/>
			...
			<xsl:apply-templates mode="do-keys" select="sch:pattern/sch:rule/sch:key | pattern/rule/key"/>
			<axsl:template match="/">
